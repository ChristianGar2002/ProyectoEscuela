	...
	if (isset($_POST['registrar'])) {
		if (strlen($_POST['nombre']) >=1){
			...
			if (($carrera_1 != $carrera_2) and ($carrera_1 != $carrera_3) and ($carrera_1 != $carrera_4) and ($carrera_2 != $carrera_3) and ($carrera_2 != $carrera_4) and ($carrera_4 != $carrera_1)){
				...
				$consulta= "INSERT INTO formulario1(apellido_paterno, apellido_materno, nombre, curp, fecha_nacimiento, edad, nacionalidad, primaria_estudio, telefono_alumno, discapacidad, promedio_primaria, certificado,tienes_hermanos, nombre_completo, ocupacion, estudio_maximo, domicilio, colonia, localidad, municipio, codigo_postal, estado, correo, telefono_tutor, telefono_familiar, carrera_1, carrera_2, carrera_3, carrera_4, fecha_registro) VALUES('$apellido_paterno', '$apellido_materno', '$nombre', '$curp', '$fecha_nacimiento','$edad', '$nacionalidad', '$primaria_estudio', '$telefono_alumno', '$discapacidad','$promedio_primaria','$certificado', '$tienes_hermanos','$nombre_completo','$ocupacion', '$estudio_maximo','$domicilio', '$colonia', '$localidad', '$municipio', '$codigo_postal', '$estado', '$correo', '$telefono_tutor', '$telefono_familiar', '$carrera_1', '$carrera_2', '$carrera_3', '$carrera_4', '$fecha_registro')";
				$resultado=mysqli_query($conex, $consulta);

				if ($resultado == true) {			
				echo '<script language="javascript">alert("Datos registrados");window.location.href="preeinscripcion_admin.php"</script>';
			}else{
				$errorMsg = "No se ha podido registrar... Intentalo nuevamente.";
		}	
			}else{
				echo '<script language="javascript">alert("Revisa tus datos y no dupliques la carrera");window.location.href="preeinscripcion_admin.php"</script>';
			}
		}
